...
- Reviewed the newest Diagnostic Extension findings again. Existing sidebar dirty/signature gating, Lorebook helper-mode churn suppression, diff-before-write UI updates, and message-edit/cache guards remain in this batch; native `/queue` polling and duplicate `/cms/banners` requests are still treated as native SpicyChat behavior and are not intercepted by QoL.
...
- Folded in the latest Diagnostic Extension follow-up state for this batch: message-edit guards/cache invalidation dedupe and scheduler counters remain enabled, sidebar reconciliation keeps its dirty/signature short-circuit, and Lorebook helper pages keep normal manager UI suppressed to avoid the previously measured no-op mutation storm. Native `/queue` and duplicate `/cms/banners` behavior remains intentionally untouched.
